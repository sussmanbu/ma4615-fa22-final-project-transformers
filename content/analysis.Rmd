...
Now, when we look at the AIC values of our two models, our abortion rate by year and abortion rate by year, political affiliation, and age, we see that our second model has a lower AIC value. So, this model that predicts abortion rate by year * political affiliation + age explains the most variation between the two models, and so we will use this as our model. We do realize that these AIC values are quite large, which is due in part by the limitations of our dataset. With the dataset that we currently have, we cannot expect an incredibly accurate model. So, if we had a dataset with complete values, we would be able to create a better model to predict abortion rate.