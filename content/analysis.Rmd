...
```{r, echo = FALSE, warning = FALSE, message = FALSE}
mod_list <- list(AbortionRateMod1, AbortionRateMod2)

mod_list %>% map_df(glance) %>% select(AIC)

```

* Now, when we look at the AIC values of our two models (the first value of 45211.95 represents the AIC of AbortionRateMod1 and the second value of 39784.73 represents the AIC of AbortionRateMod2), our abortion rate by year and abortion rate by year, political affiliation, and age, we see that our second model has a lower AIC value. So, this model that predicts abortion rate by year * political affiliation + age explains the most variation between the two models, and so we will use this as our model. We do realize that these AIC values are quite large, which is due in part by the limitations of our dataset. With the dataset that we currently have, we cannot expect an incredibly accurate model. So, if we had a dataset with complete values, we would be able to create a better model to predict abortion rate.